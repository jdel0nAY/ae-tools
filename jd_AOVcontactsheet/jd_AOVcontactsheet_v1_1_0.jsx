 ...
 *
 * CHANGES IN 1.1.0
 *   - Padding: a configurable gap in pixels between cells, applied as a gutter
 *     and as an outer margin so the spacing reads evenly. The max sheet width
 *     accounts for it, so adding padding shrinks the cells rather than the sheet.
 *   - Filenames are decoded defensively. decodeURI throws a URIError on a stray
 *     per-cent sign, which a file called "shot_50%.exr" would have triggered.
 *   - AOV names are sanitised before being built into a preset path. They come
 *     from the EXR header, so a channel named "../../x" could otherwise have sent
 *     the preset lookup outside the configured folder.
 *   - Bounds checks on the EXR header reader, and a cap on the attribute loop.
 *
...
(function jd_AOVcontactsheet() {
    ...
    var DEFAULTS = {
        ...
        layerPrefix: "",             // strip this from AOV names; empty = auto-detect
        multiNameSource: "shared",
        multiNameRegex: "([^_.]+)[._]\\d+\\.exr$",
        sourceColumnWidth: 0,        // 0 = size the column to the longest name
        maxSheetWidth: 3840,
        cellPadding: 0,              // gap between cells, and around the sheet
        addLabels: true,
        addBackground: true,
    ...
    /**
     * decodeURI throws a URIError on a lone per-cent sign, which is legal in a
     * filename - "shot_50%.exr" would have taken the script down.
     */
    function safeDecode(s) {
        try { return decodeURI(String(s)); } catch (e) { return String(s); }
    }

    /**
     * An AOV name reaches us from an EXR header or a filename, so it is untrusted
     * as far as path building goes. Single dots have to survive - an unstripped
     * layer prefix legitimately contains them - but separators and parent
     * directory hops must not.
     */
    ...
    /** encodeURI leaves # and ? alone, and both break a file URI. */
    function encodeForPath(s) {
        return encodeURI(String(s)).replace(/#/g, "%23").replace(/\?/g, "%3F");
    }
    ...
    function readInt32(buf, st) {
        if (st.pos + 4 > buf.length) {
            throw new Error("EXR header ended mid-value - the file looks truncated.");
        }
    ...
    function readExrChannels(file) {
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (++attrCount > 512) { break; }   // malformed header guard
     ...
     * name is <base><two digits>, there are at least two such siblings, and
     * either the un-suffixed <base> also exists (Arnold, Redshift) or the base
     * name says cryptomatte (Blender, which writes CryptoObject00/01/02 with no
     * un-suffixed layer). Requiring one of those two keeps ordinary AOVs ending
     * in digits - light01 beside light02 - from being swallowed by accident.
     */
    function collapseCryptoRanks(aovs) {
        ...
        function collapsible(base) {
            return names.hasOwnProperty(base) || /crypto/i.test(base);
        }

        var siblingCount = {};
        for (i = 0; i < aovs.length; i++) {
            var m = aovs[i].name.match(/^(.+?)(\d{2})$/);
            if (m && collapsible(m[1])) {
                siblingCount[m[1]] = (siblingCount[m[1]] || 0) + 1;
            }
        }

        var kept = [], dropped = [], renamed = {};
        for (i = 0; i < aovs.length; i++) {
            var mm = aovs[i].name.match(/^(.+?)(\d{2})$/);
            if (mm && collapsible(mm[1]) && siblingCount[mm[1]] >= 2) {
                // With no un-suffixed layer to fold into, promote the first rank
                // to carry the base name so there is still a cell for the set.
                if (!names.hasOwnProperty(mm[1]) && !renamed.hasOwnProperty(mm[1])) {
                    renamed[mm[1]] = true;
                    aovs[i].fullName = aovs[i].fullName || aovs[i].name;
                    aovs[i].name = mm[1];
                    kept.push(aovs[i]);
                    continue;
                }
    ...
    /**
     * An explicit prefix from the configuration window, applied to every AOV name
     * whether or not they all share it - which is the point: auto-detection can
     * only act on a prefix common to all AOVs, and it stays silent when there is
     * one AOV, or when two names would collide.
     *
     * The value is used as a regex anchored at the start, so "ViewLayer." matches
     * literally and "subimage\d+\." covers every part of a multi-part file. The
     * only thing not honoured is a match that would leave a cell with no name.
     */
    function stripConfiguredPrefix(aovs, pattern) {
        var re;
        try { re = new RegExp("^(?:" + pattern + ")"); }
        catch (e) { return { changed: false, error: e.toString() }; }

        var changed = false;
        for (var i = 0; i < aovs.length; i++) {
            var n = trimSeparators(aovs[i].name.replace(re, ""));
            if (n === "" || n === aovs[i].name) { continue; }
            if (!aovs[i].fullName) { aovs[i].fullName = aovs[i].name; }
            aovs[i].name = n;
            changed = true;
        }
    ...
    function duplicateNames(aovs) {
        var seen = {}, dupes = [];
        for (var i = 0; i < aovs.length; i++) {
            var k = aovs[i].name.toLowerCase();
            if (seen.hasOwnProperty(k)) { dupes.push(aovs[i].name); }
            seen[k] = 1;
        }
        return dupes;
    }
    ...
    /**
     * Some renderers put a layer name in front of every channel - Blender writes
     * ViewLayer.DiffDir.R, ViewLayer.Normal.X and so on. When every AOV shares
     * the same leading dot-segment it carries no information, so it is dropped.
     * Segments are whole dot-separated names, never partial strings, and the
     * result is only accepted when all names stay unique and non-empty.
     */
    function stripSharedLayerPrefix(aovs) {
        if (aovs.length < 2) { return false; }

        var lists = [], i;
        for (i = 0; i < aovs.length; i++) { lists.push(aovs[i].name.split(".")); }
    ...
    function folderNameOf(item) {
        try { return safeDecode(item.mainSource.file.parent.name); } catch (e) { return ""; }
    }

    function fileNameOf(item) {
        try { return safeDecode(item.mainSource.file.name); } catch (e) { return ""; }
    }
    ...
    function presetFileNameFor(pattern, aovName) {
        var name = String(pattern).replace(/<AOV>/g, safeNameForFile(aovName));
    ...
    function findPreset(folder, fileName, matchCase) {
        ...
        try {
            var direct = new File(folder.absoluteURI + "/" + encodeForPath(fileName));
        ...
        if (!matchCase) {
            ...
            for (var i = 0; i < all.length; i++) {
                if (safeDecode(all[i].name).toLowerCase() === fileName.toLowerCase()) { return all[i]; }
            }
        }
        return null;
    }
        ...
        if (adobe.exists) {
            ...
            try {
                kids = adobe.getFiles(function (f) {
                    return (f instanceof Folder) && /after\s*effects/i.test(safeDecode(f.name));
                }) || [];
            } catch (e2) { kids = []; }

            kids.sort(function (a, b) {
                var na = safeDecode(a.name), nb = safeDecode(b.name);
    ...
    function buildSheet(rows, settings, compBaseName) {
        ...
        // Padding is both the gutter between cells and the margin around the
        // sheet, so the spacing reads evenly. It is taken out of the width budget
        // before the cells are scaled, so adding padding shrinks the cells rather
        // than pushing the sheet past its maximum width.
        var pad = Math.max(0, Math.round(settings.cellPadding));
        var availW = settings.maxSheetWidth - pad * (cols + 1);
        if (availW < cols * 2) { availW = cols * 2; }

        var k = 1;
        if (cols * maxW > availW) { k = availW / (cols * maxW); }

        // Floor rather than round, so rounding can never push the sheet past the
        // maximum width. Height follows the cell width to keep the aspect exact.
        var cellW = Math.max(2, Math.floor(maxW * k));
        var cellH = Math.max(2, Math.round(cellW * maxH / Math.max(1, maxW)));
        var sheetW = cellW * cols + pad * (cols + 1);
        var sheetH = cellH * rowCount + pad * (rowCount + 1);
        ...
        for (i = 0; i < n; i++) {
            ...
            var cellX = pad + col * (cellW + pad);
            var cellY = pad + gridRow * (cellH + pad);
            var cx = cellX + cellW / 2;
            var cy = cellY + cellH / 2;
            ...
            var fit = Math.min(cellW / Math.max(1, row.item.width),
                               cellH / Math.max(1, row.item.height));
    ...
    function showConfig(settings) {
        ...
        var preRow = aPanel.add("group");
        preRow.add("statictext", undefined, "Strip prefix from AOV names:");
        var preField = preRow.add("edittext", undefined, settings.layerPrefix);
        preField.characters = 24;
        var preHint = aPanel.add("statictext", undefined,
            "Literal or regex, anchored at the start. e.g. ViewLayer.  or  subimage\\d+\\." +
            "   Empty = detect automatically.", { multiline: true });
        preHint.preferredSize = [500, 16];
        ...
        wRow.add("statictext", undefined, "Max sheet width (px):").preferredSize.width = 150;
        var widthField = wRow.add("edittext", undefined, String(settings.maxSheetWidth));
        widthField.characters = 6;

        var padRow = sPanel.add("group");
        padRow.add("statictext", undefined, "Padding (px):").preferredSize.width = 150;
        var padField = padRow.add("edittext", undefined, String(settings.cellPadding));
        padField.characters = 6;
        padRow.add("statictext", undefined, "gap between cells and around the sheet");
        ...
        saveBtn.onClick = function () {
            ...
            if (trim(preField.text)) {
                try { new RegExp("^(?:" + trim(preField.text) + ")"); }
                catch (e3) {
                    alert("The AOV name prefix is not a valid pattern:\n\n" + e3.toString());
                    return;
                }
            }
            ...
            var cp = parseInt(trim(padField.text), 10);
            if (isNaN(cp) || cp < 0) { cp = 0; }
            if (cp > 512) { cp = 512; }
            ...
            out = {
                ...
                layerPrefix: trim(preField.text),
    ...
    function showMain(sourceLine, rows, settings, layersMode) {
        ...
        for (i = 0; i < rows.length; i++) {
            ...
            var midText = layersMode ? (rows[i].preset ? safeDecode(rows[i].preset.name) : rows[i].expected)
                                     : rows[i].sourceName;
        ...
        for (var c = 0; c < colCount; c++) {
            ...
            for (i = start; i < end; i++) {
                ...
                var middle = layersMode ? (r.preset ? safeDecode(r.preset.name) : r.expected)
                                        : r.sourceName;
    ...
    function main() {
        ...
        if (layersMode) {
            ...
            if (trim(settings.layerPrefix)) {
                var res = stripConfiguredPrefix(baseAOVs, settings.layerPrefix);
                if (res.error) {
                    alert("The AOV name prefix is not a valid pattern:\n\n" + res.error +
                          "\n\nCarrying on without stripping it.", SCRIPT_NAME);
                } else if (res.changed) {
                    partsStripped = true;
                    prefixHow = "configured";
                }
            } else {
                partsStripped = stripPartPrefixes(baseAOVs, exrParts);
                if (stripSharedLayerPrefix(baseAOVs)) { partsStripped = true; }
                if (partsStripped) { prefixHow = "auto"; }
            }
            nameDupes = duplicateNames(baseAOVs);
        } else {
            rebuildFileAOVs();
        }
        ...
        function sourceLineText() {
            ...
            if (partsStripped) { note += ", prefix stripped (" + prefixHow + ")"; }
        ...
        var msg = 'Created "' + built.comp.name + '"\n\n' +
                  ...
                  built.comp.width + " x " + built.comp.height + " px" +
                  (settings.cellPadding > 0 ? ("  (" + settings.cellPadding + " px padding)") : "") +
                  "\nPresets applied: " + rep.applied;

        if (partsStripped) {
            msg += "\n\nPrefix stripped from the AOV names (" + prefixHow + ").";
        }
        if (nameDupes.length) {
            msg += "\n\nWarning - these names are no longer unique after stripping:\n  " +
                   nameDupes.join(", ") + "\nThey share one preset and one label.";
        }
